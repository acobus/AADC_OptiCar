...
cCrossingDetect::cCrossingDetect(const tChar* __info) : cFilter(__info)
{
	...
	SetPropertyInt(CrossingDetect_PROP_ImagecutWidthLeft, 10);
	...
	SetPropertyInt(CrossingDetect_PROP_ImagecutWidthLeftHough, 10);
...
*/
		if(m_DeadEndRightBool) TransmitCrossingSpot(CrossPoint[idx].z); //Falls rechts "zu" müssen wir eine ecke von links nehmen.
