...
ADTF_FILTER_PLUGIN("LineTracking", OID_ADTF_LineTracking, cLineTracking)
...
cLineTracking::cLineTracking(const tChar* __info) : cFilter(__info), 
    ...
    m_hEmergencyStopTimerResume(NULL)
{
    ...
    SetPropertyInt(LT_PROP_TRESHOLD, 200);
...
cLineTracking::~cLineTracking()
{
}

tResult cLineTracking::GetInterface(const tChar* idInterface,
    tVoid** ppvObject)
{
...
tUInt cLineTracking::Ref()
{
    return cFilter::Ref();
}

tUInt cLineTracking::Unref()
{
    return cFilter::Unref();
}

tVoid cLineTracking::Destroy()
{
    delete this;
}

tResult cLineTracking::Start(__exception)
{
    return cFilter::Start(__exception_ptr);
}

tResult cLineTracking::Stop(__exception)
{
...
tResult cLineTracking::Init(tInitStage eStage, __exception )
{
    ...
    if (eStage == StageFirst)
    {
        ...
        //StartCalcArea Output
        RETURN_IF_FAILED(m_oStartCalcArea.Create("Start_Calc_Area", pTypeSignalBoolValue, static_cast<IPinEventSink*> (this)));
        RETURN_IF_FAILED(RegisterPin(&m_oStartCalcArea));
        RETURN_IF_FAILED(pTypeSignalBoolValue->GetInterface(IID_ADTF_MEDIA_TYPE_DESCRIPTION, (tVoid**)&m_pStartCalcAreaOutput));
...
tResult cLineTracking::PropertyChanged(const char* strProperty)
{
...
tResult cLineTracking::ReadProperties(const tChar* strPropertyName)
{
...
tResult cLineTracking::Run(tInt nActivationCode, const tVoid* pvUserData, tInt szUserDataSize, ucom::IException** __exception_ptr)
{
...
tResult cLineTracking::Shutdown(tInitStage eStage, ucom::IException** __exception_ptr)
{
...
tResult cLineTracking::OnPinEvent(IPin* pSource, tInt nEventCode, tInt nParam1, tInt nParam2, IMediaSample* pMediaSample)
{
    ...
    if(nEventCode == IPinEventSink::PE_MediaSampleReceived)
    {
		...
		else if (pSource == &m_iDistance && m_checkLastDistance )
		{
...
tResult cLineTracking::ProcessInput(IMediaSample* pSample, tTimeStamp tsInputTime)
{
...
//        Mat matImageCutNear= image(cv::Range(m_nCurrentNearLine - 20, m_nCurrentNearLine + 20), cv::Range(m_sInputFormat.nWidth/2-X_WIDTH, m_sInputFormat.nWidth/2+X_WIDTH)).clone(); //Cut Image
		Mat matImageCutNear= image(cv::Range(m_nCurrentNearLine - 20, m_nCurrentNearLine + 20), cv::Range(m_sInputFormat.nWidth/2-X_WIDTH, m_sInputFormat.nWidth/2+X_WIDTH)).clone();
/*//Mat matImageCutNear= image(cv::Range(350 - 20, 350 + 20), cv::Range(0, m_sInputFormat.nWidth)).clone(); //Cut Image        
    ...
    */
		...
		fstream f;
		f.open("ColorTesting.dat", ios::out);
		...
		for (int i = 0; i < matNearSize.height; ++i) {
			for (int j = 0; j < matNearSize.width; ++j) {
				...
				if ( i == matNearSize.height/2) f << color << "\t";

				if (color[0] < 90 && color[1] < 90 && color[2] < 100) {
						color[0] = 0;
						color[1] = 0;
						color[2] = 0;				
				}
		...
		/*fstream f4;
		f4.open("thres2.dat",ios::out);
		f4 << matImageCutNear << "\n";
		f4.close();	*/
        medianBlur(matImageCutNear, matImageCutNear,3); // reduce bright noise with edge-preserving filter
        Canny(matImageCutNear, m_matLineCannyNear,m_nThresholdValue, m_nThresholdValue, 3, false);// Detect Edges ---------Nur Canny "funktioniert"-------
...
tResult cLineTracking::ProcessFound()
{        

    RETURN_NOERROR;
}

tResult cLineTracking::ProcessOutput()
{

    RETURN_NOERROR;
}

tResult cLineTracking::SearchHorizontalLine(){

	if (!m_checkLastDistance){
		...
		{
			if((m_lines[i][1]*180/M_PI<85) || (m_lines[i][1]*180/M_PI>95))
				{
...
tInt16 cLineTracking::GetAverage(tInt16 x, std::deque<tInt16> &Buffer)
{
	Buffer.push_back(x);
...
tInt16 cLineTracking::GetAverage2(tInt16 x, std::deque<tInt16> &Buffer)
{
	Buffer.push_back(x);
		
	float 	killer = 0.9;

	vector<float> killerVec;

	killerVec.push_back(killer);
	killerVec.push_back(1-killer);
	...
	for(size_t i=0; i<sBuffer;i++)
	{
		xav+=Buffer.at(i)*killerVec.at(i);
	}
	//xav=floor(xav/sBuffer);

	Buffer.at(AVERAGING_COUNT-1) 	= xav;
 ...
 */
tResult cLineTracking::Search(sPoint *psPoints, tUInt8 *pui8PointsCount, tUInt8 ui8Limit, cv::Mat matCannyImg, int mode)
{
    ...
    *pui8PointsCount    = 0;
	...
	if (m_goodXPoints.size()==0){
		...
		// get all point pairs with small distances
		tInt16 minDist 	= MID_DIST;
		//tInt16 minInd 	= 0;
		vector<tInt16> smallDistancesInd;
		for (size_t j = 0; j < xDistances.size(); j++){
			if (xDistances.at(j) < minDist){
				smallDistancesInd.push_back(j);
				//minDist = xDistances.at(j);
				//minInd  = j;
			}		
		}
		...
		for (size_t i = 0; i < smallDistancesInd.size(); i++){
			...
			if ( abs(m_goodXPoints.at(smallDistancesInd.at(i))-m_lastPointX) < abs(xValue - m_lastPointX) ){
				xValue = m_goodXPoints.at(smallDistancesInd.at(i));
			}
		}
...
tResult cLineTracking::LateralControl(sPoint *psPoints, tUInt8 *pui8PointsCount, tTimeStamp tsInputTime)
{
 ...
 */
tResult cLineTracking::GetSignalValue(tSignalID nSignalID, tSignalValue * pValue)
{
 ...
 */
tResult cLineTracking::ActivateSignalEvents(tSignalID nSignalID, tTimeStamp nUpdateRate)
{
 ...
 */
tResult cLineTracking::DeactivateSignalEvents(tSignalID nSignalID)
{
    __synchronized_kernel(m_oLock);
...
tResult cLineTracking::TransmitAcceleration(tFloat32 f32Acceleration, tTimeStamp tsInputTime)
{
	//if (m_bActive){	
...
tResult cLineTracking::TransmitSteeringAngle(const tFloat32 f32SteeringAngle, tTimeStamp tsInputTime)
{
...
tResult cLineTracking::TransmitSteeringAnglePT1(const tFloat32 f32SteeringAngle, tTimeStamp tsInputTime)
{
    __synchronized_obj(m_oTransmitSteerCritSection);
...
tResult cLineTracking::TransmitHeadLights(const tBool bHeadLights, tTimeStamp tsInputTime)
{
...
tResult cLineTracking::CreateAndTransmitGCL()
{
	...
	// Plot m_bActive
	strText = cString::FromBool(m_bActive);
	cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255, 255, 255).GetRGBA());	
 ...
 */
tResult cLineTracking::ChangeCurrentVelocity(IMediaSample* pMediaSample){
...
tResult cLineTracking::startCalcArea(tBool state){
	...
	cObjectPtr<IMediaSerializer> pSerializer;
	m_pStartCalcAreaOutput->GetMediaSampleSerializer(&pSerializer);
	pMediaSample->AllocBuffer(pSerializer->GetDeserializedSize());


	{
		__adtf_sample_write_lock_mediadescription(m_pStartCalcAreaOutput, pMediaSample, pCoderOutput); 

		if(!m_bStartCalcAreaOutputSet){
			pCoderOutput->GetID("bValue", m_szIDBoolValueStartCalcAreaOutput);
			pCoderOutput->GetID("ui32ArduinoTimestamp", m_szIDTimestampStartCalcAreaOutput);
			m_bStartCalcAreaOutputSet = tTrue;
		}
		
		pCoderOutput->Set(m_szIDBoolValueStartCalcAreaOutput, (tVoid*)&(state));
		pCoderOutput->Set(m_szIDTimestampStartCalcAreaOutput, (tVoid*)&nTimeStamp);
	
	}

	pMediaSample->SetTime(pMediaSample->GetTime());
	m_oStartCalcArea.Transmit(pMediaSample);

	RETURN_NOERROR;
}

tResult cLineTracking::processOverallDistance(IMediaSample* pMediaSample){
	...
	if (fDistance - m_distanceOffset > LAST_DISTANCE){
		TransmitAcceleration(0.0, _clock->GetStreamTime());
		m_bActive 	= tFalse;
		m_checkLastDistance = tFalse;
		startCalcArea(tTrue);
	}
