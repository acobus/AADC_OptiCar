...
cZebraStripes::cZebraStripes(const tChar* __info) : cFilter(__info)
{
	...
	SetPropertyInt(ZebraStripes_PROP_ImagecutHeightUp, 225);
...
/*			if(maxNear>=4 && maxFar>=4){
				...
				m_iframeCounter = 0;*/
			if (maxNear>=3){ // TODO: MAKE THIS WORK BETTER!!!
				TransmitZebraSpot(m_bufZebraNear[idxNear]-m_fDistance,m_bufZebraNear[idxNear]-m_fDistance+0.4);
				m_iframeCounter = 0;
			} else {
				// Try again if failed
				m_bufZebraNear.push_back(m_ZebraStripes[0]+m_fDistance);
