...
#define X_DEVIATION_RightCurve 80
#define X_DEVIATION_LeftStraight 25
 
#define LANE_TYPE_LEFT_UP 4
#define LANE_TYPE_LEFT_DOWN 82 
#define LANE_TYPE_RIGHT_UP 176
#define LANE_TYPE_RIGHT_DOWN 100

#define FarLineRightCurve 180
#define NearLineRightCurve 220

#define BUFFERSIZE_LANE_TYPE 5
#define BUFFERSIZE_CURVE_MODE 2
#define BUFFERSIZE_STRAIGHT_MODE 5

#define BUFFERSIZE_MAX 5


ADTF_FILTER_PLUGIN("LaneTracking", OID_ADTF_LaneTracking, cLaneTracking)

// define the ADTF property names to avoid errors 
#define LT_PROP_NEAR_LINE "Common::Near Line"
#define LT_PROP_NEAR_LINE_MAX_OFFSET "AADCLaneDetection::Near area::Max offset for near line"
#define LT_PROP_LANE_WIDTH_MIN_NEAR "AADCLaneDetection::Near area::Lane width min near"
#define LT_PROP_LANE_WIDTH_MAX_NEAR "AADCLaneDetection::Near area::Lane width max near"

#define LT_PROP_FAR_LINE "Common::Far Line"
#define LT_PROP_LANE_WIDTH_MIN_FAR "AADCLaneDetection::Far area::Lane width min far"
#define LT_PROP_LANE_WIDTH_MAX_FAR "AADCLaneDetection::Far area::Lane width max far"

#define LT_PROP_CAMERA_OFFSET "Common::Camera Offset"
...
#define LT_PROP_DRIVE_TIME "unnecessary::Drive Time in milliseconds"
#define LT_PROP_EMERGENCY_STOP_TIME "unnecessary::Emergency Stop Time in milliseconds"
...
#define LT_PROP_ImagecutWidthLeftHough "Common::ImageCut::ImagecutWidthLeftHough"
#define LT_PROP_ImagecutWidthRightHough "Common::ImageCut::ImagecutWidthRightHough"
#define LT_PROP_ImagecutHeightUpHough "Common::ImageCut::ImagecutHeightUpHough"
#define LT_PROP_ImagecutHeightDownHough "Common::ImageCut::ImagecutHeightDownHough"
#define LT_PROP_TRESHOLD "LaneTracking::Threshold"
#define LT_PROP_ThresholdValueCanny "LaneTracking::ThresholdValueCanny"
#define LT_PROP_DirectThreshold "LaneTracking::ThresholdDirect"

#define LT_PROP_ThresholdValueHough "LaneTracking::ThresholdValueHough"
#define LT_PROP_ThresholdValueHoughCurve "LaneTracking::CurveMode::ThresholdValueHoughCurve"
#define LT_PROP_DistRadiusRightUp "LaneTracking::CurveMode::DistRadiusRightImageUp"
#define LT_PROP_DistRadiusLeftUp "LaneTracking::CurveMode::DistRadiusLeftImageUp"
#define LT_PROP_ImagecutWidthLeftHoughCurve "LaneTracking::CurveMode::ImagecutWidthLeftHoughCurve"
#define LT_PROP_ImagecutWidthRightHoughCurve "LaneTracking::CurveMode::ImagecutWidthRightHoughCurve"
//#define LT_PROP_DistRadiusRightDown "LaneTracking::CurveMode::DistRadiusRightImageDown"
//#define LT_PROP_DistRadiusLeftDown "LaneTracking::CurveMode::DistRadiusLeftImageDown"
...
{
    SetPropertyInt(LT_PROP_NEAR_LINE, 200);
    ...
    SetPropertyInt(LT_PROP_FAR_LINE, 140);
    ...
    SetPropertyInt(LT_PROP_DRIVE_TIME, 0);
    ...
    SetPropertyStr(LT_PROP_ImagecutWidthRightHough NSSUBPROP_DESCRIPTION, "Cuts the Image for Hough...");

	SetPropertyInt(LT_PROP_ImagecutWidthLeftHoughCurve, 50);
	SetPropertyInt(LT_PROP_ImagecutWidthLeftHoughCurve NSSUBPROP_MIN, 0);
    SetPropertyStr(LT_PROP_ImagecutWidthLeftHoughCurve NSSUBPROP_DESCRIPTION, "Cuts the Image for Hough...");

	SetPropertyInt(LT_PROP_ImagecutWidthRightHoughCurve, 510);
	SetPropertyInt(LT_PROP_ImagecutWidthRightHoughCurve NSSUBPROP_MIN, 0);
    SetPropertyStr(LT_PROP_ImagecutWidthRightHoughCurve NSSUBPROP_DESCRIPTION, "Cuts the Image for Hough...");

	SetPropertyInt(LT_PROP_ImagecutHeightUpHough, 85);
	...
	SetPropertyInt(LT_PROP_ImagecutHeightDownHough, 245);
    ...
    SetPropertyInt(LT_PROP_DirectThreshold, 45);
    SetPropertyBool(LT_PROP_DirectThreshold NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_DirectThreshold NSSUBPROP_DESCRIPTION, "The Thresholdvalue applied first. Direct on the rgb image");

    SetPropertyInt(LT_PROP_ThresholdValueCanny, 230);
    ...
    SetPropertyInt(LT_PROP_ThresholdValueHough, 60);
    SetPropertyBool(LT_PROP_ThresholdValueHough NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_ThresholdValueHough NSSUBPROP_DESCRIPTION, "The Thresholdvalue for the Houghlines in STRAIGHT mode");

    SetPropertyInt(LT_PROP_ThresholdValueHoughCurve, 35);
    SetPropertyBool(LT_PROP_ThresholdValueHoughCurve NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_ThresholdValueHoughCurve NSSUBPROP_DESCRIPTION, "The Thresholdvalue for the Houghlines in CURVE mode");

    SetPropertyInt(LT_PROP_DistRadiusRightUp, 100);
    SetPropertyBool(LT_PROP_DistRadiusRightUp NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_DistRadiusRightUp NSSUBPROP_DESCRIPTION, "The Distance from upper left corner to mid lane in right curve mode. Kind of upper image cut for hough lines.");

    SetPropertyInt(LT_PROP_DistRadiusLeftUp, 35);
    SetPropertyBool(LT_PROP_DistRadiusLeftUp NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_DistRadiusLeftUp NSSUBPROP_DESCRIPTION, "The Distance from upper left corner to right lane in left curve mode. Kind of upper image cut for hough lines.");

/*    SetPropertyInt(LT_PROP_DistRadiusRightDown, 173);
    SetPropertyBool(LT_PROP_DistRadiusRightDown NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_DistRadiusRightDown NSSUBPROP_DESCRIPTION, "The Distance from upper left corner to mid lane in right curve mode. Kind of lower image cut for hough lines.");

    SetPropertyInt(LT_PROP_DistRadiusLeftDown, 255);
    SetPropertyBool(LT_PROP_DistRadiusLeftDown NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(LT_PROP_DistRadiusLeftDown NSSUBPROP_DESCRIPTION, "The Distance from upper left corner to right lane in left curve mode. Kind of lower image cut for hough lines.");
*/
...
tResult cLaneTracking::Init(tInitStage eStage, __exception )
{
    ...
    if (eStage == StageFirst)
    {
		...
		// Start (Input)
    	RETURN_IF_FAILED(m_iStart.Create("CircleModeBool", pTypeSignalBoolValue, static_cast<IPinEventSink*> (this)));
    	RETURN_IF_FAILED(RegisterPin(&m_iStart));
		RETURN_IF_FAILED(pTypeSignalBoolValue->GetInterface(IID_ADTF_MEDIA_TYPE_DESCRIPTION, (tVoid**)&m_pStartBoolIDInput));
        ...
        RETURN_IF_FAILED(pTypeSignalBoolValue->GetInterface(IID_ADTF_MEDIA_TYPE_DESCRIPTION, (tVoid**)&m_pCoderDescBoolStart));
		...
		// Keep Angle Input
        RETURN_IF_FAILED(m_iKeepAngle.Create("Keep_Angle", pTypeSignalBoolValue, static_cast<IPinEventSink*> (this)));
        RETURN_IF_FAILED(RegisterPin(&m_iKeepAngle));
        RETURN_IF_FAILED(pTypeSignalBoolValue->GetInterface(IID_ADTF_MEDIA_TYPE_DESCRIPTION, (tVoid**)&m_pBoolKeepAngleInput));
...
tResult cLaneTracking::ReadProperties(const tChar* strPropertyName)
{
	...
	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_ImagecutWidthLeftHoughCurve))
    {
        m_nImagecutWidthLeftHoughCurve = GetPropertyInt(LT_PROP_ImagecutWidthLeftHoughCurve);
	}

	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_ImagecutWidthRightHoughCurve))
    {
        m_nImagecutWidthRightHoughCurve = GetPropertyInt(LT_PROP_ImagecutWidthRightHoughCurve);
	}
    ...
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_CAMERA_OFFSET))
    {
        ...
        m_sLaneCenter = 320 + static_cast<tInt16> (m_f64CamOffset);
	...
	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_ThresholdValueHoughCurve))
    {
        m_nThresholdValueHoughCurve = GetPropertyInt(LT_PROP_ThresholdValueHoughCurve);
    }
	...
	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_DirectThreshold))
    {
        m_nThresholdDirect = GetPropertyInt(LT_PROP_DirectThreshold);
    }

	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_DistRadiusLeftUp))
    {
        m_nDistRadiusLEFTImageUp = GetPropertyInt(LT_PROP_DistRadiusLeftUp);
    }
/*	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_DistRadiusLeftDown))
    {
        m_nDistRadiusLEFTImageDown = GetPropertyInt(LT_PROP_DistRadiusLeftDown);
    }*/
	
	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_DistRadiusRightUp))
    {
        m_nDistRadiusRIGHTImageUp = GetPropertyInt(LT_PROP_DistRadiusRightUp);
    }
	
/*	if (NULL == strPropertyName || cString::IsEqual(strPropertyName, LT_PROP_DistRadiusRightDown))
    {
        m_nDistRadiusRIGHTImageDown = GetPropertyInt(LT_PROP_DistRadiusRightDown);
    }
*/
    RETURN_NOERROR;
}
...
tResult cLaneTracking::OnPinEvent(IPin* pSource, tInt nEventCode, tInt nParam1, tInt nParam2, IMediaSample* pMediaSample)
{
    ...
    if(nEventCode == IPinEventSink::PE_MediaSampleReceived)
    {
		...
		else if(pSource == &m_iKeepAngle){
			KeepAngle(pMediaSample);
		}
        ...
        else if(pSource == &m_oInputStart)
        {
            ...
            {   // focus for sample read lock
                __adtf_sample_read_lock_mediadescription(m_pCoderDescBoolStart,pMediaSample,pCoder);
            ...
            if (bValue)
            {
				...
				m_xGroup1Straight.clear();		// Reset all buffers!!
				m_xGroup2NotRight.clear();
				m_xGroup3Right.clear();
				m_xGroup4Left.clear();
				m_nearRightBuffer.clear();
				m_farRightBuffer.clear();
				m_nearMidBuffer.clear();
				m_farMidBuffer.clear();
				m_group3Buffer.clear();
				if(LaneType == LANE_STRAIGHT) m_sLaneCenter = 320 + static_cast<tInt16> (m_f64CamOffset);
				if(LaneType == LANE_RIGHT) m_sLaneCenter = 400 + static_cast<tInt16> (m_f64CamOffset);
				if(LaneType == LANE_LEFT) m_sLaneCenter = 240 + static_cast<tInt16> (m_f64CamOffset);				
		...
		else if(pSource == &m_iStart)
        {
            ...
            {   // focus for sample read lock
				...
				if (!m_bStartBoolIDInput) {
					pCoder->GetID("bValue", m_szTrafficCircleBoolInput);
			...
			if (!bisCircle){
				LOG_INFO(cString::Format("LT: Not Circle") );						
			} else{
				LOG_INFO(cString::Format("LT: Circle mode: %i",bisCircle) );			
			}
...
tResult cLaneTracking::ProcessInput(IMediaSample* pSample, tTimeStamp tsInputTime)
{
    ...
    else
    {
		...
		f << m_matCutHough << "\n";
		f.close();	*/	
		threshold(m_matCutHough ,m_matCutHough ,m_nThresholdDirect, 255,THRESH_TOZERO); // delete dark noise
		m_matCutHough=m_matCutHough-m_nThresholdDirect;
		/*fstream f30;
		f30.open("thres.dat",ios::out);
		f30 << m_matCutHough << "\n";
		f30.close();*/	
		...
		//if(LaneType == UNKNOWN_LANE) LOG_WARNING("!!!!!  UNKNOWN_LANE Type  !!!!!");

		if(m_HoughGroup1.empty() && m_HoughGroup2.empty() && m_HoughGroup3.empty() && m_HoughGroup4.empty())//...if we didnt find lines use aadclanetracking
		{
		/*    // Transform nearfield (Nearfield is used for lateral control)
		    ...
		    cvtColor(matImageCutFar, matImageCutFar ,CV_RGB2GRAY);// Grey Image 
			threshold(matImageCutFar,matImageCutFar, 60, 255,THRESH_TOZERO); // delete dark noise
			matImageCutFar=matImageCutFar-60;
			normalize(matImageCutFar,matImageCutFar, 0, 255, NORM_MINMAX, CV_8UC1,Mat());
			convertScaleAbs(matImageCutFar, matImageCutFar); // Konveriere in 8Bit array. Nehme aus abs
		    medianBlur(matImageCutFar, matImageCutFar,3); // reduce bright noise with edge-preserving filter
		    Canny(matImageCutFar,m_matLineCannyFar,m_nThresholdValue, m_nThresholdValueCanny, 3, false);// Detect Edges ---------Nur Canny "funktioniert"-------
...
*/
	...
	/*	fstream f2;
		f2.open("Hough.dat",ios::out);
		for(size_t i=0; i<m_lines.size();i++)
		{
			if((m_lines[i][0]>m_nDistRadiusRight))f2 << m_lines[i][0] << "," << m_lines[i][1]*180/M_PI << "\n";
		}
		f2.close();*/

		//Decide which kind of lane we have:
		if(!m_lines.empty() && !m_BoolTrafficCircle)
		{
			...
			float average = 0.0;
				for(size_t i=0; i<m_lines.size();i++)
				{					
			if((m_lines[i][1]*180/M_PI<LANE_TYPE_LEFT_DOWN) && (m_lines[i][1]*180/M_PI>LANE_TYPE_LEFT_UP)) //Eigentlich sollte die grenze bei 90°(waagerecht) sein aber die  erfahrung hat gezeigt, dass 95 besser funktioniert,weil manchmal ein paar lines über 90° hinaus kommen trotz rechtskurve.
					{
						GroupLeft.push_back(Point2f(m_lines[i][0],m_lines[i][1]));	
						average = average + m_lines[i][1]*180/M_PI;	
					}
					else if((m_lines[i][1]*180/M_PI>LANE_TYPE_RIGHT_DOWN) && (m_lines[i][1]*180/M_PI<LANE_TYPE_RIGHT_UP))
					{
			...
			if(GetAverage( GroupLeft.size(), m_LaneTypeBufferLeft,BUFFERSIZE_LANE_TYPE)>=1 && GetAverage( GroupRight.size(), m_LaneTypeBufferRight,BUFFERSIZE_LANE_TYPE)>1) 
			{
				...
				if(LaneType!=LANE_STRAIGHT)
				{
					...
					//m_group3Buffer.clear();
					m_xGroup1Straight.clear();
					m_xGroup2NotRight.clear();
					m_xGroup3Right.clear();
					m_xGroup4Left.clear();

					// near line reset in case of left mode just before
					ReadProperties(LT_PROP_NEAR_LINE); 
					ReadProperties(LT_PROP_FAR_LINE); 
				}
			...
			else
			{
				if(average<90) 
				{
					...
					if(LaneType!=LANE_RIGHT)
					{
						...
						//in right mode we cannot look that far
						m_nNearLine=NearLineRightCurve;
						m_nFarLine=FarLineRightCurve;
					}
					LaneType = LANE_RIGHT;
				}
				else if(average>90) 
		...
		if(LaneType == LANE_LEFT)
		{
			Mat m_matCutHough = image(cv::Range(m_nImagecutHeightUpHough, m_nImagecutHeightDownHough), cv::Range(m_nImagecutWidthLeftHoughCurve, m_nImagecutWidthRightHough)).clone(); //Cut Image 
		    cvtColor(m_matCutHough, m_matCutHough ,CV_RGB2GRAY);// Grey Image 
			threshold(m_matCutHough ,m_matCutHough ,m_nThresholdDirect, 255,THRESH_TOZERO); // delete dark noise
			m_matCutHough=m_matCutHough-m_nThresholdDirect;
			normalize(m_matCutHough,m_matCutHough, 0, 255, NORM_MINMAX, CV_8UC1,Mat());
			...
			HoughLines(m_matCannyHough,m_linesCurve,1,CV_PI/180,m_nThresholdValueHoughCurve,0,0);
		}
		else if(LaneType == LANE_RIGHT)
		{
			Mat m_matCutHough1 = image(cv::Range(m_nImagecutHeightUpHough, m_nImagecutHeightDownHough), cv::Range(m_nImagecutWidthLeftHough, m_nImagecutWidthRightHoughCurve)).clone(); //Cut Image 
		    cvtColor(m_matCutHough1, m_matCutHough1 ,CV_RGB2GRAY);// Grey Image 		
			threshold(m_matCutHough1 ,m_matCutHough1 ,m_nThresholdDirect, 255,THRESH_TOZERO); // delete dark noise
			m_matCutHough1=m_matCutHough1-m_nThresholdDirect;
			normalize(m_matCutHough1,m_matCutHough1, 0, 255, NORM_MINMAX, CV_8UC1,Mat());
			convertScaleAbs(m_matCutHough1, m_matCutHough1); // Konveriere in 8Bit array. Nehme aus abs
		    medianBlur(m_matCutHough1, m_matCutHough1,3); // reduce bright noise with edge-preserving filter
		    Canny(m_matCutHough1, m_matCutHough1,m_nThresholdValueCanny, m_nThresholdValueCanny, 3, false);// Detect Edges ---------Nur Canny "funktioniert"-------
			/*fstream f50;
			f50.open("canny.dat",ios::out);
			f50 << m_matCutHough1 << "\n";
			f50.close(); */

			HoughLines(m_matCutHough1,m_linesCurve,1,CV_PI/180,m_nThresholdValueHoughCurve,0,0);
		}


		SearchHough();

		//LOG_INFO(cString::Format("houghGroup1: %i, houghGroup2: %i, houghGroup3: %i ",m_HoughGroup1.size(),m_HoughGroup2.size(),m_HoughGroup3.size())); 

		if(m_HoughGroup1.empty() && m_HoughGroup2.empty() && m_HoughGroup3.empty() && m_HoughGroup4.empty())//...if we didnt find lines use aadclanetracking
		{
			if(m_DriveStraight && m_bActive) // Before changing this, ask Andi!!
			{
				TransmitAcceleration(m_f32AccelerationMin, _clock->GetStreamTime());
				TransmitSteeringAngle(0.0, _clock->GetStreamTime());
			}
			else if (m_KeepAngle && m_bActive){

				TransmitAcceleration(m_f32AccelerationMin, _clock->GetStreamTime());
				TransmitSteeringAngle(m_f32SteeringAngle, _clock->GetStreamTime());
			}
			// BACK UP
			...
			//LOG_WARNING(cString::Format("!!!!!!!!!!!!!!    aadc lane tracking    !!!!!!!!!!!!!!!!!!!!")); 
		}
		else 
		{
			// BEST LANETRACKING EVER :D
			LateralControl(m_HoughpointsNear, &m_HoughLineSizeCounterNear, tsInputTime);
    		LongitudinalControl(m_HoughpointsFar, &m_HoughLineSizeCounterFar, tsInputTime);
			//LOG_INFO(cString::Format("our lane tracking with number of info points: %i", m_HoughLineSizeCounter)); 				
		}
...
tResult cLaneTracking::SearchHough()
{	
	m_HoughLineSizeCounterNear = 0;
	m_HoughLineSizeCounterFar = 0;
	for(size_t i=0; i<m_lines.size();i++)
	{
		if(LaneType == LANE_STRAIGHT)
		{
			if((m_lines[i][1]*180/M_PI<60) && (m_lines[i][1]*180/M_PI>20))
			{
				// HoughGroup1(mid line)
				m_HoughGroup1.push_back(Point2f(m_lines[i][0],m_lines[i][1]));		
			}
			else if((m_lines[i][1]*180/M_PI<145) && (m_lines[i][1]*180/M_PI>100))
			{
	...
	for(size_t i=0; i<m_linesCurve.size();i++)
	{
		if(LaneType == LANE_RIGHT)//bei einer rechtskurve wollen wir nur die nahen lines, also die lines mit großem radius
		{
			if((m_linesCurve[i][1]*180/M_PI<83) && (m_linesCurve[i][1]*180/M_PI>40) && abs(m_linesCurve[i][0])>m_nDistRadiusRIGHTImageUp)// && abs(m_lines[i][0])<m_nDistRadiusRIGHTImageDown)
			{
				// HoughGroup3(right line)
				m_HoughGroup3.push_back(Point2f(m_linesCurve[i][0],m_linesCurve[i][1]));		
			}
		}
		else if(LaneType == LANE_LEFT)// Linkskurve
		{
			if((m_linesCurve[i][1]*180/M_PI<150) && (m_linesCurve[i][1]*180/M_PI>110) && abs(m_linesCurve[i][0])>m_nDistRadiusLEFTImageUp)// && abs(m_lines[i][0])<m_nDistRadiusLEFTImageDown)
			{
				// HoughGroup4 (mid line)
				m_HoughGroup4.push_back(Point2f(m_linesCurve[i][0],m_linesCurve[i][1]));	
			}
		}
	}

	/*fstream f1;
	f1.open("Hough1.dat",ios::out);
	for(size_t i=0; i<m_HoughGroup1.size();i++)
	{
		f1 << m_HoughGroup1[i].x << "," << m_HoughGroup1[i].y*180/M_PI << "\n";
	}
	f1.close();

	fstream f2;
	f2.open("Hough2.dat",ios::out);
	for(size_t i=0; i<m_HoughGroup2.size();i++)
	{
		f2 << m_HoughGroup2[i].x << "," << m_HoughGroup2[i].y*180/M_PI << "\n";
	}
	f2.close();

	fstream f3;
	f3.open("Hough3.dat",ios::out);
	for(size_t i=0; i<m_HoughGroup3.size();i++)
	{
		f3 << m_HoughGroup3[i].x << "," << m_HoughGroup3[i].y*180/M_PI << "\n";
	}
	f3.close();

	fstream f4;
	f4.open("Hough4.dat",ios::out);
	for(size_t i=0; i<m_HoughGroup4.size();i++)
	{
		f4<< m_HoughGroup4[i].x << "," << m_HoughGroup4[i].y*180/M_PI << "\n";
	}
	f4.close();
*/
...
//DETAILED comments can be found in case m_HoughGroup4! (it's the curve left modus case)
	...
	{	
		...
		// Counter, so we know were the new line lie with respect to the old point
		int lower1 = 0;
		int upper1 = 0;
		int lower2 = 0;
		int upper2 = 0;
		...
		for(size_t i=0; i<m_HoughGroup1.size();i++)
		{
			...
			if(pt1m.x != pt2m.x )				
			{
				...
				if (m_xGroup1Straight.empty() || abs(m_xGroup1Straight.at(0)-temp1) < X_DEVIATION_LeftStraight){
					if(temp1>x1 && temp1 < 500)
					{
						x1=temp1;
					}
				} else if (temp1 < m_xGroup1Straight.at(0)) lower1++;
				else if (temp1 > m_xGroup1Straight.at(0)) upper1++;
				
				
				if (m_xGroup1Straight.empty() || abs(m_xGroup1Straight.at(1)-temp2) < X_DEVIATION_LeftStraight){
					if(temp2>x2 && temp2 < 500)
					{
						x2=temp2;
					}
				} else if (temp2 < m_xGroup1Straight.at(1)) lower2++;
				else if (temp2 > m_xGroup1Straight.at(1)) upper2++;					
			}
		}
		
		// Set new x1 and x2 value if we didn't found good hough points
		// In equal case: go to the middle of the lane
		if (x1 == 0 && !m_xGroup1Straight.empty()){
			x1 = lower1 <= upper1 ? m_xGroup1Straight.at(0)+X_DEVIATION_LeftStraight : m_xGroup1Straight.at(0)-X_DEVIATION_LeftStraight;
		}
		if (x2 == 0 && !m_xGroup1Straight.empty()){
			x2 = lower2 <= upper2 ? m_xGroup1Straight.at(1)+X_DEVIATION_LeftStraight : m_xGroup1Straight.at(1)-X_DEVIATION_LeftStraight;
		}
		x1 = x1 < 0 ? 0 : x1;
		x2 = x2 < 0 ? 0 : x2;
		x1 = x1 > m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough ? m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough : x1;
		x2 = x2 > m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough ? m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough : x2;		

		

		m_xGroup1Straight.clear();
			
		m_xGroup1Straight.push_back(x1);
		m_xGroup1Straight.push_back(x2);
		
		m_HoughpointsNear[0].y = m_nNearLine;
		m_HoughpointsNear[0].x = GetAverage(x1, m_nearMidBuffer,BUFFERSIZE_STRAIGHT_MODE);
		m_HoughpointsFar[0].y = m_nFarLine; 
		m_HoughpointsFar[0].x = GetAverage(x2, m_farMidBuffer,BUFFERSIZE_STRAIGHT_MODE);
		m_HoughLineSizeCounterNear++;
		m_HoughLineSizeCounterFar++;

	}
	if(!m_HoughGroup2.empty() && LaneType == LANE_STRAIGHT)//!(LaneType == LANE_RIGHT))
	{
		tInt16 x1,x2;
		x1=640;
		x2=640;
		...
		for(size_t i=0; i<m_HoughGroup2.size();i++)
		{
...

			{
				...
				if (m_xGroup2NotRight.empty() || abs(m_xGroup2NotRight.at(0)-temp1) < X_DEVIATION_LeftStraight){
					if(temp1<x1 && temp1 > 150)
					{
						x1=temp1;
					}
				}else if (temp1 < m_xGroup2NotRight.at(0)) lower1++;
				else if (temp1 > m_xGroup2NotRight.at(0)) upper1++;
				
				
				if (m_xGroup2NotRight.empty() || abs(m_xGroup2NotRight.at(1)-temp2) < X_DEVIATION_LeftStraight){
					if(temp2<x2 && temp2 > 150)
					{
						x2=temp2;
					}
				}else if (temp2 < m_xGroup2NotRight.at(1)) lower2++;
				else if (temp2 > m_xGroup2NotRight.at(1)) upper2++;		
		...
		// Set new x1 and x2 value if we didn't found good hough points
		// In equal case: go to the middle of the lane
		if (x1 == 640 && !m_xGroup2NotRight.empty()){
			x1 = lower1 < upper1 ? m_xGroup2NotRight.at(0)+X_DEVIATION_LeftStraight : m_xGroup2NotRight.at(0)-X_DEVIATION_LeftStraight;
		}
		if (x2 == 640 && !m_xGroup2NotRight.empty()){
			x2 = lower2 < upper2 ? m_xGroup2NotRight.at(1)+X_DEVIATION_LeftStraight : m_xGroup2NotRight.at(1)-X_DEVIATION_LeftStraight;
		}
		x1 = x1 < 0 ? 0 : x1;
		x2 = x2 < 0 ? 0 : x2;
		x1 = x1 > m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough ? m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough : x1;
		...
		m_xGroup2NotRight.push_back(x1);
		m_xGroup2NotRight.push_back(x2);

		x1 = static_cast<tFloat32>(GetAverage(x1, m_nearRightBuffer,BUFFERSIZE_STRAIGHT_MODE));
		x2 = static_cast<tFloat32>(GetAverage(x2, m_farRightBuffer,BUFFERSIZE_STRAIGHT_MODE));

		//Send Lane Details
		vector<tFloat32> LaneDetails;
		LaneDetails.push_back(x1);
		LaneDetails.push_back(static_cast<tFloat32>(m_nNearLine));
		LaneDetails.push_back(x2);
		LaneDetails.push_back(static_cast<tFloat32>(m_nFarLine));
		TransmitLaneDetails(LaneDetails);
			
		// Be careful: if we already wrote to m_Houghpoints(..), then we don't want to overwrite those values, so write to index number [1]
		// else write to index number [0]
		if(!m_HoughGroup1.empty())
		{
			m_HoughpointsNear[1].y = m_nNearLine;//Bei 350 Pixeln berechnet deren lanetracking die spurenbreite
			m_HoughpointsNear[1].x = x1; 
			m_HoughpointsFar[1].y = m_nFarLine; 
			m_HoughpointsFar[1].x = x2;
			m_HoughLineSizeCounterNear++;
			m_HoughLineSizeCounterFar++;
		}
		else
		{
			...
			m_HoughpointsNear[0].x = x1;
			m_HoughpointsFar[0].y = m_nFarLine; 
			m_HoughpointsFar[0].x = x2;
			m_HoughLineSizeCounterNear++;
			m_HoughLineSizeCounterFar++;
		}

	}
	if(!m_HoughGroup3.empty() && LaneType == LANE_RIGHT)
	{
		tInt16 x1,x2;
		x1=0;
		x2=0;
		...
		{
			Point2f pt1m,pt2m;
			pt1m.x = cos(m_HoughGroup3[i].y)*abs(m_HoughGroup3[i].x) + 1000*(-sin(m_HoughGroup3[i].y));
			pt1m.y = sin(m_HoughGroup3[i].y)*abs(m_HoughGroup3[i].x) + 1000*(cos(m_HoughGroup3[i].y));
			pt2m.x = cos(m_HoughGroup3[i].y)*abs(m_HoughGroup3[i].x) - 1000*(-sin(m_HoughGroup3[i].y));
			pt2m.y = sin(m_HoughGroup3[i].y)*abs(m_HoughGroup3[i].x) - 1000*(cos(m_HoughGroup3[i].y));
			...
			if(pt1m.x != pt2m.x)				
			{
				...
				if (m_xGroup3Right.empty() || abs(m_xGroup3Right.at(0)-temp1) < X_DEVIATION_RightCurve){
					if(temp1>x1 && temp1<530+m_nImagecutWidthRightHough-m_nImagecutWidthRightHoughCurve)
					{
						x1=temp1;
					}
				}else if (temp1 < m_xGroup3Right.at(0)) lower1++;
				else if (temp1 > m_xGroup3Right.at(0)) upper1++;
					
				
				if (m_xGroup3Right.empty() || abs(m_xGroup3Right.at(1)-temp2) < X_DEVIATION_RightCurve){
					if(temp2>x2)
					{
						x2=temp2;
					}
				}else if (temp2 < m_xGroup3Right.at(1)) lower2++;
				else if (temp2 > m_xGroup3Right.at(1)) upper2++;					
			}
		}
		
		// Set new x1 and x2 value if we didn't found good hough points
		// In equal case: go to the middle of the lane
		if (x1 == 0 && !m_xGroup3Right.empty()){
			x1 = lower1 <= upper1 ? m_xGroup3Right.at(0)+X_DEVIATION_RightCurve : m_xGroup3Right.at(0)-X_DEVIATION_RightCurve;
		}
		if (x2 == 0 && !m_xGroup3Right.empty()){
			x2 = lower2 <= upper2 ? m_xGroup3Right.at(1)+X_DEVIATION_RightCurve : m_xGroup3Right.at(1)-X_DEVIATION_RightCurve;
		}
		x1 = x1 < m_nImagecutWidthLeftHough ? m_nImagecutWidthLeftHough : x1;
		x2 = x2 < m_nImagecutWidthLeftHough ? m_nImagecutWidthLeftHough : x2;
		x1 = x1 > m_nImagecutWidthRightHoughCurve-m_nImagecutWidthLeftHough ? m_nImagecutWidthRightHoughCurve-m_nImagecutWidthLeftHough : x1;
		x2 = x2 > m_nImagecutWidthRightHoughCurve-m_nImagecutWidthLeftHough ? m_nImagecutWidthRightHoughCurve-m_nImagecutWidthLeftHough : x2;		

		m_xGroup3Right.clear();
						
		m_xGroup3Right.push_back(x1);
		m_xGroup3Right.push_back(x2);
		
		m_HoughpointsNear[0].y = m_nNearLine;//Bei 350 Pixeln berechnet deren lanetracking die spurenbreite
		m_HoughpointsNear[0].x = GetAverage(x1,m_nearMidBuffer,BUFFERSIZE_CURVE_MODE); 
		m_HoughpointsFar[0].y = m_nFarLine; 
		m_HoughpointsFar[0].x = GetAverage(x2, m_farMidBuffer,BUFFERSIZE_CURVE_MODE);
		m_HoughLineSizeCounterNear++;
		m_HoughLineSizeCounterFar++;			
	}

	if(!m_HoughGroup4.empty() && LaneType == LANE_LEFT)
	{
		// the names of the new lane points (careful: they are averaged with the last lane points before used!)
		tInt16 x1,x2;
		x1=640;
		x2=640;
		
		// Counter, so we know were the new line lies with respect to the old point (more detailed further down)
		int lower1 = 0;
		int upper1 = 0;
		int lower2 = 0;
		int upper2 = 0;
		...
		for(size_t i=0; i<m_HoughGroup4.size();i++)
		{
			//Calculate a linear func representation for each hough line
			Point2f pt1m,pt2m;
			pt1m.x = cos(m_HoughGroup4[i].y)*m_HoughGroup4[i].x + 1000*(-sin(m_HoughGroup4[i].y));
			pt1m.y = sin(m_HoughGroup4[i].y)*m_HoughGroup4[i].x + 1000*(cos(m_HoughGroup4[i].y));
			pt2m.x = cos(m_HoughGroup4[i].y)*m_HoughGroup4[i].x - 1000*(-sin(m_HoughGroup4[i].y));
			pt2m.y = sin(m_HoughGroup4[i].y)*m_HoughGroup4[i].x - 1000*(cos(m_HoughGroup4[i].y));
			...
			if(pt1m.x != pt2m.x )				
			{
				...
				// The next lane point x1 is not allowed to be farther away from the last lane point m_xGroup4Left.at(0) than X_DEVIATION
				// is that not fullfilled the next point x1 is calculated as m_xGroup4Left.at(0)+-X_DEVIATION, 
				// depending on, whether lower1 or upper1 has more votes
				if (m_xGroup4Left.empty() || abs(m_xGroup4Left.at(0)-temp1) < X_DEVIATION_LeftStraight){
					if(temp1<x1 && temp1 > 170-m_nImagecutWidthLeftHoughCurve)
					{
						x1=temp1;
					}
				}else if (temp1 < m_xGroup4Left.at(0)) lower1++;
				else if (temp1 > m_xGroup4Left.at(0)) upper1++;
					
				// same for far point x2
				if (m_xGroup4Left.empty() || abs(m_xGroup4Left.at(1)-temp2) < X_DEVIATION_LeftStraight){
					if(temp2<x2 && temp2 > 90-m_nImagecutWidthLeftHoughCurve)
					{
						x2=temp2;
					}
				}else if (temp2 < m_xGroup4Left.at(1)) lower2++;
				else if (temp2 > m_xGroup4Left.at(1)) upper2++;					
			}
		}
		
		// Set new x1 and x2 value if we didn't found good hough points
		// In (unlikely) equal case: go to the middle of the lane
		if (x1 == 640 && !m_xGroup4Left.empty()){
			x1 = lower1 <= upper1 ? m_xGroup4Left.at(0)+X_DEVIATION_LeftStraight : m_xGroup4Left.at(0)-X_DEVIATION_LeftStraight;
		}
		if (x2 == 640 && !m_xGroup4Left.empty()){
			x2 = lower2 <= upper2 ? m_xGroup4Left.at(1)+X_DEVIATION_LeftStraight : m_xGroup4Left.at(1)-X_DEVIATION_LeftStraight;
		}

		// important: don't leave the range of the image cut!
		x1 = x1 < m_nImagecutWidthLeftHoughCurve ? m_nImagecutWidthLeftHoughCurve : x1;
		x2 = x2 < m_nImagecutWidthLeftHoughCurve ? m_nImagecutWidthLeftHoughCurve : x2;
		x1 = x1 > m_nImagecutWidthRightHough-m_nImagecutWidthLeftHoughCurve ? m_nImagecutWidthRightHough-m_nImagecutWidthLeftHoughCurve : x1;
		x2 = x2 > m_nImagecutWidthRightHough-m_nImagecutWidthLeftHoughCurve ? m_nImagecutWidthRightHough-m_nImagecutWidthLeftHoughCurve : x2;		

		m_xGroup4Left.clear();
						
		m_xGroup4Left.push_back(x1);
		m_xGroup4Left.push_back(x2);
		
		// Store averaged lane info
		m_HoughpointsNear[0].y = m_nNearLine;//Bei 350 Pixeln berechnet deren lanetracking die spurenbreite
		m_HoughpointsNear[0].x = GetAverage(x1+m_nImagecutWidthLeftHoughCurve,m_nearRightBuffer,BUFFERSIZE_CURVE_MODE); 
		m_HoughpointsFar[0].y = m_nFarLine; 
		m_HoughpointsFar[0].x = GetAverage(x2+m_nImagecutWidthLeftHoughCurve, m_farRightBuffer,BUFFERSIZE_CURVE_MODE);
		m_HoughLineSizeCounterNear++;
		m_HoughLineSizeCounterFar++;		
	}
...
tInt16 cLaneTracking::GetAverage(tInt16 x, std::deque<tInt16> &Buffer, tUInt bufferSize)
{
	Buffer.push_back(x);
	size_t sBuffer = Buffer.size();
	if(sBuffer>BUFFERSIZE_MAX)
	{
		Buffer.pop_front();
		sBuffer--;
	}
	...
	tInt16 xav=0;
	tInt16 numElem = 0;
	for(int i=sBuffer-1; i>(int)(sBuffer-bufferSize-1);i--)
	{
		if (i<0) break;
		xav+=Buffer.at(i);
		numElem++;
	}
	xav=floor(xav/numElem);
	return xav;
}
...
tResult cLaneTracking::LateralControl(sPoint *psPoints, tUInt8 *pui8PointsCount, tTimeStamp tsInputTime)
{
	...
	if (!m_KeepAngle || m_DriveStraight){ // New angle has to be calculated
		if(*pui8PointsCount == 2)
		{
		    tInt16 i16LaneWidthCalc = psPoints[1].x - psPoints[0].x;
		   // LOG_INFO(cString::Format("Point0: %i, Point1: %i, LaneWidth calc: %i", psPoints[0].x, psPoints[1].x, i16LaneWidthCalc));
		    tInt nLaneCenterCalc = psPoints[0].x + (static_cast<tInt>(i16LaneWidthCalc/2));

		    if ((i16LaneWidthCalc > m_i16LaneWidthMinNear) && (i16LaneWidthCalc < m_i16LaneWidthMaxNear) && abs(nLaneCenterCalc - m_sLaneCenter) < MAX_DEVIATION)
		    {
		        m_i16LaneWidth = i16LaneWidthCalc;
		        m_sLaneCenter = nLaneCenterCalc;
		        m_nBlindCounter = 0;
		    }
		    else
		    {
				//LOG_WARNING(cString::Format("LF: LaneWidthNear not correct: i16LaneWidthCalc | m_i16LaneWidthMinNear | m_i16LaneWidthMaxNear | abs(nLaneCenterCalc - m_sLaneCenter) | MAX_DEVIATION = %i | %i | %i | %i | %i", i16LaneWidthCalc, m_i16LaneWidthMinNear, m_i16LaneWidthMaxNear, abs(nLaneCenterCalc - m_sLaneCenter), MAX_DEVIATION) );
		        m_nBlindCounter++;
		    }        
		}
		else if(*pui8PointsCount == 3)
		{
		    ...
		    if((i16Width1 > m_i16LaneWidthMinNear) && (i16Width1 < m_i16LaneWidthMaxNear) && abs(nLaneCenterCalc1 - m_sLaneCenter) < MAX_DEVIATION)
		    {
		        m_i16LaneWidth = i16Width1;
		        m_sLaneCenter = nLaneCenterCalc1;
		        m_nBlindCounter = 0;
		    }
		    else if((i16Width2 > m_i16LaneWidthMinNear) && (i16Width2 < m_i16LaneWidthMaxNear) && abs(nLaneCenterCalc2 - m_sLaneCenter) < MAX_DEVIATION)
		    {
		        m_i16LaneWidth = i16Width2;
		        m_sLaneCenter = nLaneCenterCalc2;
		        m_nBlindCounter = 0;
		    }
		    else
		    {
		...
		else if(*pui8PointsCount == 1)
		{
			...
			//LOG_INFO(cString::Format("abs(m_nCenterFromLeft - m_sLaneCenter): %i", abs(m_nCenterFromLeft - m_sLaneCenter)));
			//LOG_INFO(cString::Format("abs(m_nCenterFromRight - m_sLaneCenter): %i", abs(m_nCenterFromRight - m_sLaneCenter)));
			if(LaneType == LANE_RIGHT)
			{
				m_nCenterFromRight = psPoints[0].x + (static_cast<tInt>(m_i16LaneWidth/2));
				m_sLaneCenter = m_nCenterFromRight;
				m_nBlindCounter = 0;
			}
		    else if(LaneType == LANE_LEFT)
			{
				m_nCenterFromLeft = psPoints[0].x - (static_cast<tInt>(m_i16LaneWidth/2));
				m_sLaneCenter = m_nCenterFromLeft;
				m_nBlindCounter = 0;
			}
				...
				m_nCenterFromRight = psPoints[0].x - (static_cast<tInt>(m_i16LaneWidth/2));
				m_nCenterFromLeft = psPoints[0].x + (static_cast<tInt>(m_i16LaneWidth/2));
				if(abs(m_nCenterFromLeft - m_sLaneCenter) < MAX_DEVIATION)
				{
				    m_sLaneCenter = m_nCenterFromLeft;
				    m_nBlindCounter = 0;
				}
				else if(abs(m_nCenterFromRight - m_sLaneCenter) < MAX_DEVIATION)
				{
				    m_sLaneCenter = m_nCenterFromRight;
				    m_nBlindCounter = 0;
					//LOG_INFO(cString::Format("2"));
				}
				else
				{
				    m_nBlindCounter++;
					//LOG_INFO(cString::Format("3")); 
		...
		//if (m_sLaneCenter < 0 || m_sLaneCenter > m_sInputFormat.nWidth)
		//{
		//    m_sLaneCenter = static_cast<tInt16> (m_sInputFormat.nWidth / 2);
		//}

		
		//PID - Controller for Steering Angle
		m_i16Error = m_sLaneCenter - m_sPlaceToBe.x;
	...
	//      LOG_INFO(cString::Format("Error smaller: LaneCenter %d Place2Be %d Steering %f error_val: %d error_sum: %d", m_LaneCenter.x, m_PlaceToBe.x, m_SteeringAngle, error, e_sum).GetPtr());
		...
		********************************************/
		...
		LOG_INFO(cString::Format("Keeping angle = %f", m_f32SteeringAngle) );
	}
...
tResult cLaneTracking::LongitudinalControl(sPoint *psPointsFar, tUInt8 *pui8PointsCountFar, tTimeStamp tsInputTime)
{
	if (m_DriveStraight){
		...
		if(*pui8PointsCountFar == 2)
		{
		    ...
		    //LOG_INFO(cString::Format("FarLaneWidth calc: %i", m_i16FarLaneWidthCalc ));
		    ...
		    if ((m_i16FarLaneWidthCalc > m_i16LaneWidthMinFar) && (m_i16FarLaneWidthCalc < m_i16LaneWidthMaxFar))
		    {
				...
				//LOG_WARNING(cString::Format("LaneWidth far not correct: m_i16FarLaneWidthCalc | m_i16LaneWidthMinFar | m_i16LaneWidthMaxFar = %i | %i | %i", m_i16FarLaneWidthCalc, m_i16LaneWidthMinFar, m_i16LaneWidthMaxFar) );
			}
		...
		else if(*pui8PointsCountFar == 1)
		{
			...
			{
				 tInt nFarCenterLeft = psPointsFar[0].x + (static_cast<tInt>(m_i16FarLaneWidth/2));
				//LOG_INFO(cString::Format("psPointsFar[0].x + (static_cast<tInt>(m_i16FarLaneWidth/2)) = %i", psPointsFar[0].x + (static_cast<tInt>(m_i16FarLaneWidth/2))));

				 tInt nFarCenterRight = psPointsFar[0].x - (static_cast<tInt>(m_i16FarLaneWidth/2));
				//LOG_INFO(cString::Format("psPointsFar[0].x - (static_cast<tInt>(m_i16FarLaneWidth/2)) = %i", psPointsFar[0].x - (static_cast<tInt>(m_i16FarLaneWidth/2))));

	 			//LOG_INFO(cString::Format("abs(nFarCenterRight - m_i16FarLaneCenter): %i", abs(nFarCenterRight - m_i16FarLaneCenter)));
				//LOG_INFO(cString::Format("abs(nFarCenterLeft - m_i16FarLaneCenter): %i", abs(nFarCenterLeft - m_i16FarLaneCenter)));
		...
		//LOG_INFO(cString::Format("m_i16FarLaneCenter = %i" , m_i16FarLaneCenter) ) ;
	...
	//        tInt16 i16Difference = abs(m_i16FarLaneCenter - m_sPlaceToBe.x);
		    tInt16 i16Difference = abs(m_i16FarLaneCenter - m_sLaneCenter);
...
/*    else if(abs(m_i16FarLaneCenter - m_sLaneCenter) <= m_nAccelerationFarNearDiff)    // This parameter determines how early the car brakes before and accelerates after a curve
    {
         m_f32AccelerateOut = m_f32AccelerationMax;
    }
    else if(abs(m_i16FarLaneCenter - m_sLaneCenter) > m_nAccelerationFarNearDiff)
    {
         m_f32AccelerateOut = m_f32AccelerationMin;
    }
*/
   // LOG_INFO(cString::Format("Acceleration: %f", m_f32AccelerateOut));
    if (m_bActive)
    {
/*	fstream fff;
	fff.open("LF_Acceleration_Min", ios::out|ios::app);
	fff << m_f32AccelerationMin << "\n";
	fff.close();*/
...
/*	fstream f;
	f.open("LTErrorDetect.dat", ios::out|ios::app);
	f << m_DriveStraight << "\t" << m_KeepAngle << "\t" << f32SteeringAngle << "\n";	
	f.close();*/
...
tResult cLaneTracking::CreateAndTransmitGCL()
{
	...
	cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWRECT, m_nImagecutWidthLeftHough, m_nImagecutHeightUpHough, m_nImagecutWidthRightHough, m_nImagecutHeightDownHough);
...
/*    //cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWRECT, 0, m_nFarLine - 20, m_sInputFormat.nWidth, m_nFarLine + 20);
	...
	{	 
		cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255,10,10).GetRGBA());
		for(size_t i=0; i<m_HoughGroup3.size();i++)
		{
			float rho = m_HoughGroup3[i].x;
			float theta = m_HoughGroup3[i].y;
			Point pt1,pt2;
			double a = cos(theta), b = sin(theta);
			double x0 = a*rho, y0 = b*rho;
			pt1.x = cvRound(x0 + 1000*(-b));
	...
	else if(LaneType == LANE_LEFT)
	{
		...
		for(size_t i=0; i<m_HoughGroup4.size();i++)
		{
			float rho = m_HoughGroup4[i].x;
			float theta = m_HoughGroup4[i].y;
			Point pt1,pt2;
			double a = cos(theta), b = sin(theta);
			double x0 = a*rho, y0 = b*rho;
			pt1.x = cvRound(x0 + 1000*(-b));
...
*/
// ------------ IMPORTANT plot hough lines responsible for LANE TYPE CHOICE ----------------
		...
		for(size_t i=0; i<GroupLeft.size();i++)
		{
			...
			if(LaneType == LANE_LEFT) cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWLINE, pt2.x+m_nImagecutWidthLeftHoughCurve, pt2.y+m_nImagecutHeightUpHough, pt1.x+m_nImagecutWidthLeftHoughCurve, pt1.y+m_nImagecutHeightUpHough);
			else cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWLINE, pt2.x, pt2.y+m_nImagecutHeightUpHough, pt1.x, pt1.y+m_nImagecutHeightUpHough);
		}
...
//  ----------------------------------------------------------
   ...
   // cGCLWriter::StoreCommand(pc, GCL_CMD_FILLRECT, m_sLaneCenter - m_i16LaneWidthMaxNear/2, m_nCurrentNearLine, m_sLaneCenter + m_i16LaneWidthMaxNear/2, m_nCurrentNearLine + 5);
   // cGCLWriter::StoreCommand(pc, GCL_CMD_FILLRECT, m_sLaneCenter - m_i16LaneWidthMinNear/2, m_nCurrentNearLine - 5, m_sLaneCenter + m_i16LaneWidthMinNear/2, m_nCurrentNearLine);
    ...
    cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255,150,0).GetRGBA());
    cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWLINE, m_sLaneCenter, m_nCurrentNearLine - 20, m_sLaneCenter, m_nCurrentNearLine + 20);
	...
	cString strText = cString::FromInt32(LaneType);
	cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255, 255, 255).GetRGBA());	
	cGCLWriter::StoreCommand(pc, GCL_CMD_TEXT, 10, 20, strText.GetLength());
	cGCLWriter::StoreData(pc, strText.GetLength(), strText.GetPtr()); 

	// Temporary: Plot m_sLaneCenter
	strText = cString::FromInt32(m_sLaneCenter);
	cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255, 255, 255).GetRGBA());	
	cGCLWriter::StoreCommand(pc, GCL_CMD_TEXT, 560, 400, strText.GetLength());
	cGCLWriter::StoreData(pc, strText.GetLength(), strText.GetPtr()); 

	// Plot Acceleration
	strText = cString::FromFloat64(static_cast<tFloat64>(m_f32AccelerateOut));
	cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255, 255, 255).GetRGBA());	
	cGCLWriter::StoreCommand(pc, GCL_CMD_TEXT, 10, 400, strText.GetLength());
	cGCLWriter::StoreData(pc, strText.GetLength(), strText.GetPtr());
...
tResult cLaneTracking::CreateAndTransmitGCL2()
{
	...
	{	 
		cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(0, 153, 153).GetRGBA());		
		cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWRECT, m_nImagecutWidthLeftHough, m_nImagecutHeightUpHough, m_nImagecutWidthRightHoughCurve, m_nImagecutHeightDownHough);
	...
	{
		cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(0, 153, 153).GetRGBA());		
		cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWRECT, m_nImagecutWidthLeftHoughCurve, m_nImagecutHeightUpHough, m_nImagecutWidthRightHough, m_nImagecutHeightDownHough);
		...
		for(size_t i=0; i<m_HoughGroup4.size();i++)
		{
			...
			pt2.x = cvRound(x0 - 1000*(-b));
			pt2.y = cvRound(y0 - 1000*(a));
			cGCLWriter::StoreCommand(pc, GCL_CMD_DRAWLINE, pt2.x+m_nImagecutWidthLeftHoughCurve, pt2.y+m_nImagecutHeightUpHough, pt1.x+m_nImagecutWidthLeftHoughCurve, pt1.y+m_nImagecutHeightUpHough);
		}
	}
...
tResult cLaneTracking::ChangeCurrentVelocity(IMediaSample* pMediaSample){
	...
	if (fMinVel > fMaxVel){
		...
		if (fMinVel == 0.0 && m_f32AccelerationMin > 0.0 && fMaxVel == 0.0 && m_f32AccelerationMax > 0.0) TransmitAcceleration(0.0f, _clock->GetStreamTime());
...
/*
 * keeping the angle now (or not)
 */
tResult cLaneTracking::KeepAngle(IMediaSample* pMediaSample){
...

	{
		...
		if (!m_bKeepAngleSet){
			...
			pCoder->GetID("bValue", m_szIDBoolValueKeepAngleInput);
			m_bKeepAngleSet 		= tTrue;

		}

   		pCoder->Get(m_szIDBoolValueKeepAngleInput, (tVoid*)&bValue); 

	}

	m_KeepAngle 		= bValue;  

	LOG_INFO(cString::Format("LF: KeepAngleReceived: %i", m_KeepAngle) );
