...
cParkDetection::cParkDetection(const tChar* __info) : cFilter(__info)
{
	...
	m_lotDim[0][1] = 0.78;
	// cross parking
	m_lotDim[1][0] = 0.525;// ;0.54;
	m_lotDim[1][1] = 0.425;//;0.41;
  ...
  */  
	...
	SetPropertyInt(ParkDetection_PROP_ImagecutHeightDown, 480);
	...
	SetPropertyFloat(ParkDetection_PROP_ThresholdValueCanny, 120);
...
{	
	...
	if( m_ui8InitCtrl<20)
	{
		...
		while(i<m_Points.size())
		{
			...
			{
				if(abs(corner1.x-inter3d.x)>0.09)LOG_WARNING(cString::Format("Punkt außerhalb!"));
...
///////////////////////////
	...
	else if(ParkPoint[idx].z > 2.3 || ParkPoint[idx].z < 0.6){
		//LOG_WARNING(cString::Format("ParkD: No valid parking lot found!"));
		// Try again if failed
		//m_bActive = tTrue;
 	}
/*	else if(ParkPoint.size()<3 && ParkPoint[idx].z>1.0)
	{
		...
		LOG_WARNING(cString::Format("ParkD: Parking Lot found, but only one. I need at least two!") );
		m_bActive = tTrue;
	}*/
	else
	{	
		...
		if(ParkPoint.size()>0) TransmitParkingSpot(temp[0]);

	}
	//LOG_INFO(cString::Format("ParkD: Finished Searching") ); 

	fstream ff;
	ff.open("parkTicToc", ios::out|ios::app);
	ff << _clock->GetStreamTime() - tictoc << "\n";
	ff.close();
	RETURN_NOERROR;
}
