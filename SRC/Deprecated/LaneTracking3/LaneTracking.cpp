    ...
    m_hEmergencyStopTimerResume(NULL)
{
	...
	SetPropertyInt(LT_PROP_ImagecutHeightUpHough, 260);
	...
	SetPropertyInt(LT_PROP_ImagecutHeightDownHough, 400);
    ...
    SetPropertyFloat(LT_PROP_ThresholdValueCanny, 40);
    ...
    SetPropertyFloat(LT_PROP_ThresholdValueHough, 60);
		...
		if(!m_lines.empty())
		{
			...
			float average = 0;
				for(size_t i=0; i<m_lines.size();i++)
				{
					...
					if((m_lines[i][1]*180/M_PI>95 || (m_lines[i][1]*180/M_PI<85)))
					{
						if((m_lines[i][1]*180/M_PI<85)) //Eigentlich sollte die grenze bei 90°(waagerecht) sein aber die  erfahrung hat gezeigt, dass 95 besser funktioniert,weil manchmal ein paar lines über 90° hinaus kommen trotz rechtskurve.
						{
							GroupLeft.push_back(Point2f(m_lines[i][0],m_lines[i][1]));		
						}
						else if((m_lines[i][1]*180/M_PI>95))
						{
							GroupRight.push_back(Point2f(m_lines[i][0],m_lines[i][1])); 	
						}
					}
				}
				average = average/m_lines.size();

			if(GroupLeft.size()>0 && GroupRight.size()>0) LaneType = LANE_STRAIGHT;
			else
			{
				if(average<90) LaneType = LANE_RIGHT;
				if(average>90) LaneType = LANE_LEFT;			
			}		
		}
...
//create linear function with houghlines
	if(!m_HoughGroup1.empty() && !(LaneType == LANE_RIGHT))
		{	
			tInt16 x1,x2;
			x1=0;
			x2=0;
...
/*
				...
				if(pt1r.x == pt2r.x || pt1r.y == pt2r.y )				
				{
				}
				else
				{
				...
				if(LaneType == LANE_LEFT)
					{
						m_HoughpointsNear[0].y = m_nNearLine;//Bei 350 Pixeln berechnet deren lanetracking die spurenbreite
						m_HoughpointsNear[0].x = (m_nNearLine-m_nImagecutHeightUpHough-b2)/m2;
					}
				else if (LaneType == LANE_STRAIGHT)
				{
					if(!m_HoughGroup1.empty())
					{
						m_HoughpointsNear[1].y = m_nNearLine;//Bei 350 Pixeln berechnet deren lanetracking die spurenbreite
						m_HoughpointsNear[1].x = (m_nNearLine-m_nImagecutHeightUpHough-b2)/m2;//Hier findet die Berechnung nur bei m_nNearLine-m_nImagecutHeightUpHough weil wir das bild bei m_nImagecutHeightUpHough cutten
						m_HoughpointsFar[1].y = m_nFarLine; 
						m_HoughpointsFar[1].x = (m_nFarLine-m_nImagecutHeightUpHough-b2)/m2;
						m_HoughLineSizeCounter++;
					}
					else
					{
						...
						m_HoughpointsNear[0].x = (m_nNearLine-m_nImagecutHeightUpHough-b2)/m2;
						m_HoughpointsFar[0].y = m_nFarLine; 
						m_HoughpointsFar[0].x = (m_nFarLine-m_nImagecutHeightUpHough-b2)/m2;
						m_HoughLineSizeCounter++;
					}
				}
				}
