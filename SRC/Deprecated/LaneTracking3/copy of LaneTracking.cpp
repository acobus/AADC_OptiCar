...
tResult cLaneTracking::ProcessInput(IMediaSample* pSample, tTimeStamp tsInputTime)
{
    ...
    else
    {
        ...
        Mat matImageCutNear= image(cv::Range(m_nCurrentNearLine - 20, m_nCurrentNearLine + 20), cv::Range(WIDTH_MIN_NEAR, m_sInputFormat.nWidth)).clone(); //Cut Image    
		...
		fstream f1;
		f1.open("nearMat.dat",ios::out);
		f1 << m_matLineCannyNear;
		f1.close();
    
        // Transform farfield (farfield is used for longitudinal control)
        Mat matImageCutFar= image(cv::Range(m_nFarLine - 20, m_nFarLine + 20), cv::Range(WIDTH_MIN_FAR, m_sInputFormat.nWidth)).clone(); //Cut Image        
...
tResult cLaneTracking::ChangeCurrentVelocity(IMediaSample* pMediaSample){
	...
	tFloat32 fMinVel = -2.0;
	tFloat32 fMaxVel = -2.0;
	...
	if (fMinVel < 0.0 || fMaxVel < 0.0){
		LOG_WARNING(cString::Format("Velocity-Values must not be negative!"));
	}
	else if (fMinVel > 10.0 || fMaxVel > 10.0){
		LOG_WARNING(cString::Format("Velocity_Values must not be bigger than 10 m/s!"));
	}
	else if (fMinVel > fMaxVel){
		LOG_WARNING(cString::Format("fMinVel > fMaxVel not allowed!"));
	}
	else {
		m_f32AccelerationMin 	= fMinVel;
		m_f32AccelerationMax 	= fMaxVel;
	}
