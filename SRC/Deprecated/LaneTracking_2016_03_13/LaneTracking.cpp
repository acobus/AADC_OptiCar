    ...
    m_hEmergencyStopTimerResume(NULL)
{
    ...
    SetPropertyInt(LT_PROP_ThresholdValueHoughCurve, 45);
...
*/
	...
	/*	fstream f2;
		...
		if(m_HoughGroup1.empty() && m_HoughGroup2.empty() && m_HoughGroup3.empty() && m_HoughGroup4.empty())//...if we didnt find lines use aadclanetracking
		{
			if (m_DriveStraight && m_bActive){

				TransmitAcceleration(m_f32AccelerationMin, 0);
				TransmitSteeringAngle(0.0, 0);
			}
			// BACK UP
...
{	
	...
	f4.close();
*/
		// don't bother about the blind counter. It works anyway.
/*		if(m_HoughGroup1.empty() && LaneType == LANE_STRAIGHT)
		...
		if(m_HoughGroup2.empty() && LaneType == LANE_STRAIGHT)
		{
			m_nearRightBuffer.clear();
			m_farRightBuffer.clear();
		}
	...
	{	
		...
		for(size_t i=0; i<m_HoughGroup1.size();i++)
		{
			...
			if(pt1m.x != pt2m.x )				
			{
				...
				if (m_xGroup1Straight.empty() || abs(m_xGroup1Straight.at(0)-temp1) < X_DEVIATION){
				...
				if (m_xGroup1Straight.empty() || abs(m_xGroup1Straight.at(1)-temp2) < X_DEVIATION){
		...
		if (x1 == 0 && !m_xGroup1Straight.empty()){
			x1 = lower1 <= upper1 ? m_xGroup1Straight.at(0)+X_DEVIATION : m_xGroup1Straight.at(0)-X_DEVIATION;
		}
		if (x2 == 0 && !m_xGroup1Straight.empty()){
			x2 = lower2 <= upper2 ? m_xGroup1Straight.at(1)+X_DEVIATION : m_xGroup1Straight.at(1)-X_DEVIATION;
		}
		x1 = x1 < 0 ? 0 : x1;
		x2 = x2 < 0 ? 0 : x2;
	...
	if(!m_HoughGroup2.empty() && LaneType == LANE_STRAIGHT)//!(LaneType == LANE_RIGHT))
	{
		...
		for(size_t i=0; i<m_HoughGroup2.size();i++)
		{
...

			{
				...
				if (m_xGroup2NotRight.empty() || abs(m_xGroup2NotRight.at(0)-temp1) < X_DEVIATION){
				...
				if (m_xGroup2NotRight.empty() || abs(m_xGroup2NotRight.at(1)-temp2) < X_DEVIATION){
		...
		if (x1 == 640 && !m_xGroup2NotRight.empty()){
			x1 = lower1 < upper1 ? m_xGroup2NotRight.at(0)+X_DEVIATION : m_xGroup2NotRight.at(0)-X_DEVIATION;
		}
		if (x2 == 640 && !m_xGroup2NotRight.empty()){
			x2 = lower2 < upper2 ? m_xGroup2NotRight.at(1)+X_DEVIATION : m_xGroup2NotRight.at(1)-X_DEVIATION;
		}
		x1 = x1 < 0 ? 0 : x1;
		x2 = x2 < 0 ? 0 : x2;
		x1 = x1 > m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough ? m_nImagecutWidthRightHough-m_nImagecutWidthLeftHough : x1;
		...
		LaneDetails.push_back(static_cast<tFloat32>(GetAverage(x1, m_nearRightBuffer)));
		LaneDetails.push_back(static_cast<tFloat32>(m_nNearLine));
		LaneDetails.push_back(static_cast<tFloat32>(GetAverage(x2, m_farRightBuffer)));
	...
	if(!m_HoughGroup3.empty() && LaneType == LANE_RIGHT)
	{
		...
		for(size_t i=0; i<m_HoughGroup3.size();i++)
		{
			...
			if(pt1m.x != pt2m.x)				
			{
				...
				if (m_xGroup3Right.empty() || abs(m_xGroup3Right.at(0)-temp1) < X_DEVIATION){
				...
				if (m_xGroup3Right.empty() || abs(m_xGroup3Right.at(1)-temp2) < X_DEVIATION){
					if(temp2>x2)
					{
						x2=temp2;
					}
		...
		if (x1 == 0 && !m_xGroup3Right.empty()){
			x1 = lower1 <= upper1 ? m_xGroup3Right.at(0)+X_DEVIATION : m_xGroup3Right.at(0)-X_DEVIATION;
		}
		if (x2 == 0 && !m_xGroup3Right.empty()){
			x2 = lower2 <= upper2 ? m_xGroup3Right.at(1)+X_DEVIATION : m_xGroup3Right.at(1)-X_DEVIATION;
		}
		...
		m_xGroup3Right.push_back(x1-RightCurveOffset);
	...
	if(!m_HoughGroup4.empty() && LaneType == LANE_LEFT)
	{
		...
		for(size_t i=0; i<m_HoughGroup4.size();i++)
		{
			...
			if(pt1m.x != pt2m.x )				
			{
				...
				if (m_xGroup4Left.empty() || abs(m_xGroup4Left.at(0)-temp1) < X_DEVIATION){
				...
				if (m_xGroup4Left.empty() || abs(m_xGroup4Left.at(1)-temp2) < X_DEVIATION){
		...
		if (x1 == 640 && !m_xGroup4Left.empty()){
			x1 = lower1 <= upper1 ? m_xGroup4Left.at(0)+X_DEVIATION : m_xGroup4Left.at(0)-X_DEVIATION;
		}
		if (x2 == 640 && !m_xGroup4Left.empty()){
			x2 = lower2 <= upper2 ? m_xGroup4Left.at(1)+X_DEVIATION : m_xGroup4Left.at(1)-X_DEVIATION;
		}
		...
		m_xGroup4Left.push_back(x1+LeftCurveOffset);
		m_xGroup4Left.push_back(x2);
		
		// Store averaged lane info
		m_HoughpointsNear[0].y = m_nNearLine;//Bei 350 Pixeln berechnet deren lanetracking die spurenbreite
		m_HoughpointsNear[0].x = GetAverage(x1+m_nImagecutWidthLeftHoughCurve,m_nearRightBuffer); 
		m_HoughpointsFar[0].y = m_nFarLine; 
		m_HoughpointsFar[0].x = GetAverage(x2+m_nImagecutWidthLeftHoughCurve, m_farRightBuffer);
		m_HoughLineSizeCounterNear++;
		m_HoughLineSizeCounterFar++;		
	}
...
tResult cLaneTracking::LateralControl(sPoint *psPoints, tUInt8 *pui8PointsCount, tTimeStamp tsInputTime)
{
    ...
    else if(*pui8PointsCount == 1)
    {
		...
		//LOG_INFO(cString::Format("abs(m_nCenterFromLeft - m_sLaneCenterNear.x): %i", abs(m_nCenterFromLeft - m_sLaneCenterNear.x)));
		//LOG_INFO(cString::Format("abs(m_nCenterFromRight - m_sLaneCenterNear.x): %i", abs(m_nCenterFromRight - m_sLaneCenterNear.x)));
...
//  ----------------------------------------------------------
   ...
   // cGCLWriter::StoreCommand(pc, GCL_CMD_FILLRECT, m_sLaneCenterNear.x - m_i16LaneWidthMinNear/2, m_nCurrentNearLine - 5, m_sLaneCenterNear.x + m_i16LaneWidthMinNear/2, m_nCurrentNearLine);
	...
	// Temporary: Plot m_sLaneCenterNear.x
	strText = cString::FromInt32(m_sLaneCenterNear.x);
	cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255, 255, 255).GetRGBA());	
