    ...
    m_hEmergencyStopTimerResume(NULL)
{
	...
	SetPropertyInt(LT_PROP_ImagecutHeightUpHough, 70);
	...
	SetPropertyInt(LT_PROP_ImagecutHeightDownHough, 255);
    ...
    if(nEventCode == IPinEventSink::PE_MediaSampleReceived)
    {
        ...
        else if(pSource == &m_oInputStart)
        {
            ...
            if (bValue)
            {
                ...
                if(true)//m_hStopTimerNegative == NULL && m_hStopTimerZero == NULL && m_nDriveTime > 0)
                {
...
*/
		...
		if(!m_lines.empty())
		{
			...
			float average = 0.0;
				for(size_t i=0; i<m_lines.size();i++)
				{					
					if((m_lines[i][1]*180/M_PI<60)) //Eigentlich sollte die grenze bei 90°(waagerecht) sein aber die  erfahrung hat gezeigt, dass 95 besser funktioniert,weil manchmal ein paar lines über 90° hinaus kommen trotz rechtskurve.
					{
						GroupLeft.push_back(Point2f(m_lines[i][0],m_lines[i][1]));	
						average = average + m_lines[i][1]*180/M_PI;	
					}
					else if((m_lines[i][1]*180/M_PI>120))
					{
...
//		LOG_INFO(cString::Format("LaneType: %i",LaneType)); // OutputLanetype
		...
		{
			if(true)//(m_ui8NearPointsCount != 0) (Andi)
			{
				LateralControl(m_asAllpointsNear, &m_ui8NearPointsCount, tsInputTime);
	    		LongitudinalControl(m_asAllpointsFar, &m_ui8FarPointsCount, tsInputTime);
			}
		...
		} else if (m_DriveStraight && m_bActive){
	...
	for(size_t i=0; i<m_lines.size();i++)
		{
			if((m_lines[i][1]*180/M_PI<60) && (m_lines[i][1]*180/M_PI>35))
				...
				else if((m_lines[i][1]*180/M_PI<83) && (m_lines[i][1]*180/M_PI>65))
				{
...
//Send Lane Details
			...
			if(LaneType == LANE_LEFT)
			{
				...
				// Dont't use this at the moment, so don't transmit anything
			}
...
tResult cLaneTracking::LongitudinalControl(sPoint *psPointsFar, tUInt8 *pui8PointsCountFar, tTimeStamp tsInputTime)
{
	...
	if (m_DriveStraight){
		...
		else if (m_nBlindCounter > 30 && !m_DriveStraight) //(Andi)
		{
...
tResult cLaneTracking::TransmitAcceleration(tFloat32 f32Acceleration, tTimeStamp tsInputTime)
{
	...
	fff.close(); */
		...
		m_accSpeed = f32Acceleration;
		//LOG_INFO(cString::Format("LF speed = %f",f32Acceleration));
...
/*
   ...
   // cGCLWriter::StoreCommand(pc, GCL_CMD_FILLRECT, m_sLaneCenterNear.x - m_i16LaneWidthMinNear/2, m_nCurrentNearLine - 5, m_sLaneCenterNear.x + m_i16LaneWidthMinNear/2, m_nCurrentNearLine);
	...
	cString strText2 = cString::FromFloat64(m_accSpeed);
	cGCLWriter::StoreCommand(pc, GCL_CMD_FGCOL, cColor(255, 255, 255).GetRGBA());	
...
tResult cLaneTracking::ChangeCurrentVelocity(IMediaSample* pMediaSample){
	...
	if (fMinVel != m_f32AccelerationMin && fMaxVel != m_f32AccelerationMax) LOG_INFO(cString::Format("LF (Wtd): fMinVel: %f, fMaxVel: %f", fMinVel, fMaxVel) );
	...
	fstream f;
	f.open("LTVelocityInput", ios::out| ios::app);
	f << cString::Format("LF (Wtd): fMinVel: %f, fMaxVel: %f", fMinVel, fMaxVel) << "\n";
	f << cString::Format("LF (Curr): fMinVel: %f, fMaxVel: %f", m_f32AccelerationMin, m_f32AccelerationMax) << "\n";
	f.close();
