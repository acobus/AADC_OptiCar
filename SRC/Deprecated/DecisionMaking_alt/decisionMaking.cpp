		...
		IMediaSample* pMediaSample)
{
	...
	if (nEventCode == IPinEventSink::PE_MediaSampleReceived)
	{
		...
		else if (pSource == &m_iDistance && !m_EmergencyBreak && m_DMActive && (m_checkDistance || m_checkLastDistance) )
		{
		...
		if (!m_EmergencyBreak && m_DMActive && !m_currentlyOvertaking &&!m_checkLastDistance){

			if (pSource == &m_iRoadSign)
			{
				processFoundRoadSign(pMediaSample);
			}
			else if (pSource == &m_iFoundParkingSpot)
			{
...
tResult cDecisionMaking::DMwork(IMediaSample* pMediaSample){
	...
	if (m_DMActive){
		if (m_CurrentManeuvre == ACTION_PULL_OUT_LEFT){
			if (!m_currentlyPullOut) sendStatusPullOut(CROSS_LEFT);
		}
	...
	else 	startModuleByID(STATE_HOLD);
...
tResult cDecisionMaking::processEmergencyBreak(IMediaSample* pMediaSample){
	...
	if (m_EmergencyBreak != bValue){
		...
		if (m_EmergencyBreak){ 
			...
			if (m_HoldWhileEmergency) 	startModuleByID(STATE_HOLD);
			else						startModuleByID(STATE_LANE_FOLLOWING);
	...
	if (!m_processingRoadSign){
		...
		if (identifier != -1){	
			...
			if(size > ROAD_SIGN_MIN_SIZE){
				...
				// Turning
				else if ( m_CurrentManeuvre == ACTION_LEFT || m_CurrentManeuvre == ACTION_RIGHT || m_CurrentManeuvre == ACTION_STRAIGHT){
					...
					// Crossing-Detection: GiveWay
						...
						// TODO: a.t.m. implement this as a give way crossroad
						m_CurrentCrossingSign 	= MARKER_ID_GIVEWAY;
						startModuleByID(STATE_TURNING);
					}
					else{
...
LOG_INFO(cString::Format("DM: ManeuverCode = %i", intValue) );
	...
	// There was an error or we have finished driving:
	if(m_CurrentManeuvre == ACTION_UNKNOWN){
		if (lastManeuvre == ACTION_PARALLEL_PARKING || lastManeuvre == ACTION_CROSS_PARKING){
			startModuleByID(STATE_HOLD);
			m_DMActive 	= tFalse;
		} else m_checkLastDistance = tTrue; // Drive one meter at the end
	}
...
tResult cDecisionMaking::processOverallDistance(IMediaSample* pMediaSample){
	...
	if (m_checkLastDistance){
		if (m_lastDistanceCheckpoint == 0.0){
			m_lastDistanceCheckpoint = fDistance;
		} else if (m_lastDistanceCheckpoint + LAST_DISTANCE >= fDistance){
			m_lastDistanceCheckpoint = 0.0;
			m_checkLastDistance = tFalse;
			startModuleByID(STATE_HOLD);
			m_DMActive 	= tFalse; // Finished after driving a last meter
		}
	} else{
		...
		if (m_referenceDistance > 0.0 && m_overallDistance > (m_referenceDistance + DRIVE_SLOW_DIST)){
...
tResult cDecisionMaking::processSteeringAngle(IMediaSample* pMediaSample){
	...
	// If we found a certain sign: start searching if wheels are approximately parallel!
	if (m_currentSteering < 90+STEERING_ERROR && m_currentSteering > 90-STEERING_ERROR){
		// Parking
		if (m_ParkingMode > 0){
...
tResult cDecisionMaking::processFoundCrossing(IMediaSample* pMediaSample){
	...
	if ( distance > 0 && (m_CurrentManeuvre == ACTION_LEFT || m_CurrentManeuvre == ACTION_RIGHT || m_CurrentManeuvre == ACTION_STRAIGHT) ) {
		...
		//startModuleByID(STATE_HOLD);
		sendStatusTurning(tTrue, distance, crossType, m_CurrentCrossingSign, m_CurrentManeuvre);
	}
	else{
...
tResult cDecisionMaking::ProcessParkLotType(IMediaSample* pMediaSample){
	...
	if (isCrossPark)	sendStatusPullOut(CROSS_RIGHT);
	else 				sendStatusPullOut(PARALLEL_RIGHT);
...
tResult cDecisionMaking::processFinishedManeuvre_Parking(IMediaSample* pMediaSample){
	...
	if (finished){
		...
		// Todo (increment maneuvre, start next maneuvre, free foundRoadSignSection, set overtaking allowed, set LF Velocity(?) <- erst nach dem ausparken? )
	}
...
tResult cDecisionMaking::startModuleByID(int state){
	...
	// Cross Parking
	if ( (state != STATE_CROSS_PARKING && m_CrossParkActive) || !m_DMActive)		m_CrossParkActive =  tFalse;
	else if (state == STATE_CROSS_PARKING && !m_CrossParkActive) 					{m_CrossParkActive 	= tTrue; m_checkDistance = tTrue; m_ParkingMode = ACTION_CROSS_PARKING; overtakingAllowed(m_OvertakingAlowed_RoadSign, tFalse, m_OvertakingAlowed_Pedestrain);}

	// Parallel Parking
	if ( (state != STATE_PARALLEL_PARKING && m_ParallelParkActive) || !m_DMActive)	m_ParallelParkActive = tFalse;
	else if (state == STATE_PARALLEL_PARKING && !m_ParallelParkActive) 				{m_ParallelParkActive = tTrue; m_checkDistance = tTrue; m_ParkingMode = ACTION_PARALLEL_PARKING; overtakingAllowed(m_OvertakingAlowed_RoadSign, tFalse, m_OvertakingAlowed_Pedestrain);}

	// Turning
	if ( (state != STATE_TURNING && m_TurningActive) || !m_DMActive)				m_TurningActive = tFalse;
	else if (state == STATE_TURNING && !m_TurningActive) 							{m_TurningActive = tTrue; m_checkDistance = tTrue; m_CurrentlyCrossing = tTrue; overtakingAllowed(m_OvertakingAlowed_RoadSign, tFalse, m_OvertakingAlowed_Pedestrain);}
	
	if ( (state != STATE_ZEBRA && m_ZebraActive) || !m_DMActive) 					m_ZebraActive = tFalse;
	else if (state == STATE_ZEBRA && !m_ZebraActive) 								{m_ZebraActive = tTrue; m_checkDistance = tTrue; searchForZebraStripes(); overtakingAllowed(m_OvertakingAlowed_RoadSign, m_OvertakingAlowed_Maneuvre, tFalse);}
...
tResult cDecisionMaking::sendStatusHold(tBool status){
...

	{
		...
		if (!m_bStateHoldOutputSet){
			...
			pCoder->GetID("ui32Timestamp", m_szIDTimestampStateHoldOutput);
...
tResult cDecisionMaking::sendStatusCrossPark(tBool status, tFloat32 pDistance){
...

	{
		...
		if (!m_bCrossParkOutputSet){
			...
			pCoder->GetID("ui32Timestamp", m_szIDTimestampStateCrossParkOutput);
...
tResult cDecisionMaking::sendStatusParallelPark(tBool status, tFloat32 pDistance){
...

	{
		...
		if (!m_bParallelParkOutputSet){
			...
			pCoder->GetID("ui32Timestamp", m_szIDTimestampStateParallelParkOutput);
 ...
 */
tResult cDecisionMaking::sendStatusPullOut(tInt32 status){

	m_currentlyPullOut 	= tTrue;
 ...
 */
tResult cDecisionMaking::sendStatusTurning(tBool status, tFloat pDistance, tInt32 pCrossType, tInt16 pCrossingSign, tInt32 pManeuvre){
	...
	tBool	 	bValue 			= status;
	tFloat32 	velocity 		= m_currentSpeed;
	tFloat32 	distance 		= pDistance - TURNING_OFFSET < 0 ? 0.001 : pDistance - TURNING_OFFSET;
	...
	else if (pCrossingSign == MARKER_ID_STOPANDGIVEWAY || pCrossingSign == MARKER_ID_GIVEWAY){
		giveWay 	= GIVEWAY;
	}
	else{
...

	{
		...
		if (!m_bTurningOutputSet){
			...
			pCoder->GetID("ui32Timestamp", m_szIDStateTurningTimestampOutput);
...
tResult cDecisionMaking::incrementManeuvre(){
...

	{
		...
		if (!m_bIncrementManeuvreOutputSet){
			...
			pCoder->GetID("ui32Timestamp", m_szIDTimestampIncrementManeuvreOutput);
...
tResult cDecisionMaking::searchForParkingSpot(tBool status, tInt32 ID) {
	...
	// Only send if the state is true
	if (status){	

		setVelocityLF(ACTION_VELOCITY, ACTION_VELOCITY);			
 ...
 */
tResult cDecisionMaking::searchForZebraStripes(){
